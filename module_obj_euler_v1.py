...
STANDARDIZED ON SAVINGS RATE (s) to fix policy inversion.
"""
import torch
import numpy as np
...
class DefineEulerObjective:
    ...
    def get_euler_loss_method2(self, x_batch, n_mc_samples, dist_a_mid):
        ...
        # [逻辑统一] Output 0 is SAVINGS RATE (s)
        ...
        # [逻辑统一] a' = s * W (与 Adapter 一致)
        x_a1 = s_0 * wealth_0
        a_max = self.ranges[1][1]
        x_a1 = torch.min(x_a1, torch.tensor(a_max, device=self.device))
        ...
        def get_euler_ratio_sample():
            ...
            x_z1 = module_basic_v1.bounded_log_normal_samples(
                config.mu_z, config.sigma_z, z_min, z_max, n_batch).unsqueeze(1).to(self.device)
            ...
            s_1 = torch.sigmoid(nn_logits_next[:, 0].unsqueeze(1)) # Next Savings Rate
            
            x_c1 = (1 - s_1) * wealth_1
            x_c1_safe = torch.maximum(x_c1, torch.tensor(1e-5, device=self.device))
            
            # Euler Ratio: beta * R * (c'/c)^-sigma
